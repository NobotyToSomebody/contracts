...
contract BucketManager is Ownable {
	...
	mapping (string => Status) public buckets;
    ...
    function _getName(string memory name) internal pure returns (string memory){
        bytes memory nameBytes = bytes(name);
        require(nameBytes.length < 18 && nameBytes.length > 0, "length of name should < 18 and > 0");
        for (uint i=0;i <nameBytes.length;i++){
            require(isLowercaseLetter(nameBytes[i]) || isDigit(nameBytes[i]) || nameBytes[i] == ',',"bucket name can only include lowercase letters, numbers, commas");
        }
        return name;
    }
    ...
    function createPolicy(
        ...
        require (buckets[name] == Status.Success, "The bucket of the given and name is not created");
		_createPolicy(name,createPolicyData,_callbackGasLimit,_failureHandleStrategy);
    }
    ...
    function _createPolicy(
        ...
        PackageQueue.FailureHandleStrategy _failureHandleStrategy
            ...
            bytes memory _callbackData = abi.encode(name,dataHash);
            CmnStorage.ExtraData memory _extraData = CmnStorage.ExtraData({
                appAddress: address(this),
                refundAddress: msg.sender,
                failureHandleStrategy: _failureHandleStrategy,
    ...
    function _policyGreenfieldCall(
        ...
        (string memory name, bytes32 dataHash) = abi.decode(callbackData,(string,bytes32));    
        if (status == 0) {
            policies[dataHash] = Status.Success;
        }else if(status == 1){
            policies[dataHash] = Status.Failed;
        }      
        emit CreatePolicy(name, dataHash, status);  
    }   
    ...
    function getBucketStatus(string memory name)public view returns(Status) {
